...
assert(VisualTab, "Передайте VisualTab при вызове visual.lua!")
  ...
  -- === SETTINGS ===
    ...
    local traceSettings = {
        ...
        mode = "Lazer", -- Default only Neon/Lazer, Legit убран
        lifetime = 0.5, -- default seconds
    }
    local logSettings = {
        enabled = false,
    ...
    WorldBox:AddDropdown("BulletTraceMode",{Values={"Neon","Lazer"},Default="Lazer",Text="Bullet Trace Mode",Callback=function(val)traceSettings.mode=val end}) -- Legit убран
    WorldBox:AddSlider("BulletTraceLifetime",{Text="Life time",Default=0.5,Min=0,Max=20,Rounding=2,Callback=function(val)traceSettings.lifetime=val end})
...
-- + Object ESP оптимизированный, всё работает только если включено, throttle, кеш, Drawing не копятся
------------------------------------------------------------
...
local function clearOffscreenDrawings()
        ...
        if model:IsA("Model") and model ~= LocalPlayer.Character then
            ...
            if head and root then
                ...
                if dist <= offscreenSettings.maxDist then
                    ...
                    local isSleep = (offscreenSettings.sleepcheck and (model:FindFirstChild("AnimationController") and model.AnimationController:GetPlayingAnimationTracks()[1] and model.AnimationController:GetPlayingAnimationTracks()[1].Animation.AnimationId == "rbxassetid://13280887764"))
                    if not isAI and not isSleep then
                        ...
                        if not onScreen or vec.Z < 0 then
                            table.insert(result, {model = model, position = root.Position, dist = dist, name = name})
                        end
                    end
                end
            end
...
RunService.RenderStepped:Connect(function()
    if not offscreenSettings.enabled or not offscreenSettings.arrow then
        clearOffscreenDrawings()
        return
    end
    if tick() - lastOffscreenUpdate < 0.04 then return end
    ...
    for i, target in ipairs(targets) do
        ...
        local thisColor = offscreenSettings.arrowColor or Color3.fromRGB(255, 200, 60):Lerp(Color3.fromRGB(255, 70, 70), t)
        ...
        if offscreenSettings.weapon then lblStr = lblStr .. "\n" .. tostring(GetPlayerWeaponType and GetPlayerWeaponType(target.model) or "None") end
        if lblStr ~= "" then
...
------------------------------------------------------------
-- WORLD VISUALS (No Grass, No Leaves, Clouds, Ambient, Always Day, Remove Fog, Skybox)
-- Всё работает только если включено
------------------------------------------------------------
...
local function setClouds(enabled, color)
    if not enabled then
        if cloudsObject then cloudsObject.Enabled = false end
        return
    end
    ensureTerrain()
...
------------------------------------------------------------
-- OBJECT ESP ОПТИМИЗИРОВАННЫЙ
------------------------------------------------------------

local objectEspLabels = {} -- {id = Drawing text}
local lastObjectEspUpdate = 0

local function getEntityMap()
    for _, v in pairs(getgc(true)) do
        if typeof(v) == "table" and rawget(v, "EntityMap") then
            return v.EntityMap
        end
    end
end
...
local function objectEspUpdate()
    if not objectEspSettings.enabled then
        cleanupLabels(objectEspLabels)
        return
    end
    if tick() - lastObjectEspUpdate < 0.15 then return end
    lastObjectEspUpdate = tick()
    local entityMap = getEntityMap()
    if not entityMap then cleanupLabels(objectEspLabels) return end
    local Camera = workspace.CurrentCamera
    local seen = {}
    for id, entity in pairs(entityMap) do
        ...
        local dist = entity.pos and (Camera.CFrame.Position - entity.pos).Magnitude or math.huge
        if entity.type == "DroppedItem" and objectEspSettings.dropped.enabled and dist <= objectEspSettings.maxDistance then
            show = true
            color = objectEspSettings.dropped.color
            labelText = (objectEspSettings.name and (entity.item or "Dropped") or "")
            if objectEspSettings.distance then
                labelText = (labelText~="" and labelText.." " or "") .. tostring(math.floor(dist)).."m"
            end
        elseif entity.type == "Corpse" and objectEspSettings.corpse.enabled and dist <= objectEspSettings.maxDistance then
            show = true
            color = objectEspSettings.corpse.color
            labelText = "Corpse"
            if objectEspSettings.distance then
                labelText = labelText.." "..tostring(math.floor(dist)).."m"
            end
        elseif entity.type == "ATV" and objectEspSettings.atv.enabled and dist <= objectEspSettings.maxDistance then
            show = true
            color = objectEspSettings.atv.color
            labelText = "ATV"
            if objectEspSettings.distance then
                labelText = labelText.." "..tostring(math.floor(dist)).."m"
            end
        elseif entity.type == "Stone" and objectEspSettings.stone.enabled and dist <= objectEspSettings.maxDistance then
            show = true
            color = objectEspSettings.stone.color
            labelText = "Stone"
            if objectEspSettings.distance then
                labelText = labelText.." "..tostring(math.floor(dist)).."m"
            end
        elseif entity.type == "Nitrate" and objectEspSettings.nitrate.enabled and dist <= objectEspSettings.maxDistance then
            show = true
            color = objectEspSettings.nitrate.color
            labelText = "Nitrate"
            if objectEspSettings.distance then
                labelText = labelText.." "..tostring(math.floor(dist)).."m"
            end
        elseif entity.type == "Iron" and objectEspSettings.iron.enabled and dist <= objectEspSettings.maxDistance then
            show = true
            color = objectEspSettings.iron.color
            labelText = "Iron"
            if objectEspSettings.distance then
                labelText = labelText.." "..tostring(math.floor(dist)).."m"
            end
        elseif entity.type == "Copter" and objectEspSettings.copter.enabled and dist <= objectEspSettings.maxDistance then
            show = true
            color = objectEspSettings.copter.color
            labelText = "Copter"
            if objectEspSettings.distance then
                labelText = labelText.." "..tostring(math.floor(dist)).."m"
            end
        elseif entity.type == "Car" and objectEspSettings.car.enabled and dist <= objectEspSettings.maxDistance then
            show = true
            color = objectEspSettings.car.color
            labelText = "Car"
            if objectEspSettings.distance then
                labelText = labelText.." "..tostring(math.floor(dist)).."m"
            end
        end
        if show and entity.pos then
            seen[id] = true
            if not objectEspLabels[id] then
                local label = Drawing.new("Text")
                label.Size = 13
                label.Outline = true
                label.Center = true
                objectEspLabels[id] = label
            end
            local label = objectEspLabels[id]
            screenPos, onScreen = Camera:WorldToViewportPoint(entity.pos)
            label.Visible = onScreen
            label.Text = labelText
            label.Position = Vector2.new(screenPos.X, screenPos.Y)
            label.Color = color
        else
            if objectEspLabels[id] then objectEspLabels[id].Visible = false end
        end
    end
    for id, label in pairs(objectEspLabels) do
        if not seen[id] then
            label.Visible = false
            label:Remove()
            objectEspLabels[id] = nil
        end
    end
end

local RunService = game:GetService("RunService")
local origHandProps, origItemProps = {}, {}

local function applyHighlight(part, color)
    -- Всегда удаляем старые highlights
...
local function setHandChams(part)
    local id = part:GetDebugId()
    -- Сохраняем оригинальные свойства только один раз
    if not origHandProps[id] then
        origHandProps[id] = {material = part.Material, color = part.Color}
    end
    -- Сброс к оригиналу перед применением нового материала
    part.Material = origHandProps[id].material
    part.Color = origHandProps[id].color
    removeHighlights(part)
    if chamsSettings.hand then
        if chamsSettings.handMat == "Chams" then
            applyHighlight(part, chamsSettings.handColor)
        else
            part.Material = Enum.Material[chamsSettings.handMat]
            part.Color = chamsSettings.handColor
        end
    end
end

local function setItemChams(part)
    local id = part:GetDebugId()
    if not origItemProps[id] then
        origItemProps[id] = {material = part.Material, color = part.Color}
    end
    -- Сброс к оригиналу перед применением нового материала
    part.Material = origItemProps[id].material
    part.Color = origItemProps[id].color
    removeHighlights(part)
    if chamsSettings.item then
        if chamsSettings.itemMat == "Chams" then
            applyHighlight(part, chamsSettings.itemColor)
        else
            part.Material = Enum.Material[chamsSettings.itemMat]
            part.Color = chamsSettings.itemColor
        end
    end
end

local function updateHandChams()
    local ignore = workspace:FindFirstChild("Const")
    ignore = ignore and ignore:FindFirstChild("Ignore")
    ...
    for _, name in ipairs({"LeftHand", "RightHand"}) do
        local hand = fpsarms:FindFirstChild(name)
        if hand and hand:IsA("BasePart") then
            setHandChams(hand)
        end
    end

    local fake = fpsarms:FindFirstChild("Fake")
    if fake then
        for _, name in ipairs({"c_LeftLowerArm", "c_RightLowerArm"}) do
...
getgenv().TridentLibrary:OnUnload(function()
    for id, _ in pairs(origHandProps) do origHandProps[id] = nil end
    for id, _ in pairs(origItemProps) do origItemProps[id] = nil end
end)
------------------------------------------------------------
-- BULLET TRACE через Beam (tail fade-out без кусков, плавно, Neon/Lazer, фикс лимита кейпоинтов)
------------------------------------------------------------
local bulletTraces = {}
local RunService = game:GetService("RunService")

-- Генерация NumberSequence: fade-out с 65% до конца, не больше 16 кейпоинтов!
local function makeTailFadeSequence(fade)
    local points = {}
    local numKeypoints = 16 -- безопасный лимит!
    for i = 0, numKeypoints do
        local t = i / numKeypoints
        local value
        if t < 0.65 then
            value = 0
        else
            -- fade начинается от 0.65 до конца
            local fadeT = (t - 0.65) / (1 - 0.65)
            value = fadeT * fade
        end
        table.insert(points, NumberSequenceKeypoint.new(t, value))
    end
    return NumberSequence.new(points)
end

local function createBeamLaser(startPos, endPos, color, mode, width, lifetime)
    -- Attachments (не двигаются)
    local partFrom = Instance.new("Part")
    partFrom.Anchored = true
    partFrom.CanCollide = false
    partFrom.Transparency = 1
    partFrom.Size = Vector3.new(0.1,0.1,0.1)
    partFrom.Position = startPos
    partFrom.Parent = workspace

    local partTo = Instance.new("Part")
    partTo.Anchored = true
    partTo.CanCollide = false
    partTo.Transparency = 1
    partTo.Size = Vector3.new(0.1,0.1,0.1)
    partTo.Position = endPos
    partTo.Parent = workspace

    local attFrom = Instance.new("Attachment")
    attFrom.Parent = partFrom
    ...
    if mode == "Neon" then
        ...
        beam.Width0 = width * 0.85
        beam.Width1 = width * 0.85
        beam.Transparency = makeTailFadeSequence(0)
        beam.CurveSize0 = 0
        beam.CurveSize1 = 0
        beam.Parent = partFrom
        table.insert(beams, beam)
    elseif mode == "Lazer" then
        -- Цветной glow
        local beamGlow = Instance.new("Beam")
        beamGlow.Attachment0 = attFrom
        beamGlow.Attachment1 = attTo
        beamGlow.FaceCamera = false
        beamGlow.Color = ColorSequence.new(color)
        beamGlow.LightEmission = 0.7
        beamGlow.Width0 = width
        beamGlow.Width1 = width
        beamGlow.Transparency = makeTailFadeSequence(0)
        beamGlow.CurveSize0 = 0
        beamGlow.CurveSize1 = 0
        beamGlow.Parent = partFrom
        table.insert(beams, beamGlow)
        -- Белый core
        local beamCore = Instance.new("Beam")
        beamCore.Attachment0 = attFrom
        beamCore.Attachment1 = attTo
        beamCore.FaceCamera = false
        beamCore.Color = ColorSequence.new(Color3.new(1,1,1))
        beamCore.LightEmission = 1
        beamCore.Width0 = width * 0.45
        beamCore.Width1 = width * 0.45
        beamCore.Transparency = makeTailFadeSequence(0.05)
        beamCore.CurveSize0 = 0
        beamCore.CurveSize1 = 0
        beamCore.Parent = partFrom
        table.insert(beams, beamCore)
    end

    -- Плавный fade-out, только конец исчезает!
    local fadeSteps = 30
    local fadeTime = lifetime or 0.5
    for i = 1, fadeSteps do
        task.delay((fadeTime/fadeSteps) * i, function()
            local fade = i/fadeSteps
            for _,beam in ipairs(beams) do
                if mode == "Lazer" and beam.Color.Keypoints[1].Value == Color3.new(1,1,1) then
                    beam.Transparency = makeTailFadeSequence(0.05 + 0.95*fade)
                else
                    beam.Transparency = makeTailFadeSequence(fade)
                end
            end
        end)
    end
    ...
    task.delay(fadeTime + 0.1, function()
        if partFrom then partFrom:Destroy() end
        if partTo then partTo:Destroy() end
    end)
end

local function createBulletBeam(part)
    ...
    local function drawTrace()
        ...
        if (currPos - prevPos).Magnitude > 0.01 then
            local width = (traceSettings.mode == "Neon") and 0.18 or 0.12
            createBeamLaser(prevPos, currPos, traceSettings.color, traceSettings.mode, width, traceSettings.lifetime)
            prevPos = currPos
        end
    end
    conn = RunService.RenderStepped:Connect(drawTrace)
    part.Destroying:Connect(function()
        if not finished then
            local width = (traceSettings.mode == "Neon") and 0.18 or 0.12
            createBeamLaser(prevPos, part.Position, traceSettings.color, traceSettings.mode, width, traceSettings.lifetime)
            finished = true
            if conn then conn:Disconnect() end
        end
    end)
...
local function createArrowBeam(part)
    local prevPos = part.Position
    local conn
    local finished = false
    local function drawTrace()
...
local function updateBulletTraces()
    ...
    if not ignore then return end
    for _, obj in ipairs(ignore:GetChildren()) do
        if obj.Name == "Arrow" and traceSettings.enabled then
            if not bulletTraces[obj] then
                bulletTraces[obj] = true
                createArrowBeam(obj)
            end
        elseif obj.Name == "Bullet" and traceSettings.enabled then
            if not bulletTraces[obj] then
                bulletTraces[obj] = true
                createBulletBeam(obj)
            end
        end
    end
end
...
workspace.DescendantAdded:Connect(function(child)
    if (child.Name == "Arrow" or child.Name == "Bullet") and traceSettings.enabled then
        task.wait(0.03)
        updateBulletTraces()
    end
end)
RunService.RenderStepped:Connect(function()
    if traceSettings.enabled then
        updateBulletTraces()
    end
end)
------------------------------------------------------------
